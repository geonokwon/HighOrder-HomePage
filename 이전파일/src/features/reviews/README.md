...
### 개요
관리자가 고객 후기에 답변할 수 있는 댓글 기능이 추가되었습니다.

### 주요 특징
- **관리자 전용**: JWT 토큰 인증을 통해 관리자만 댓글 작성/삭제 가능
- **실시간 업데이트**: 댓글 작성 후 즉시 반영
- **Clean Architecture**: Domain → Application → Infrastructure → Presentation 계층 구조 유지

### API 엔드포인트
- `GET /api/reviews/[id]/comments` - 댓글 목록 조회
- `POST /api/reviews/[id]/comments` - 댓글 작성 (관리자 전용)
- `DELETE /api/reviews/[id]/comments/[commentId]` - 댓글 삭제 (관리자 전용)

### 사용 방법
1. 관리자 페이지에서 로그인
2. 리뷰 페이지로 이동
3. 각 리뷰 카드 하단에서 댓글 작성 폼 확인
4. 댓글 작성 후 제출
